...
<div class="form-box">
    <div class="form-box-head">
        <h4 class="title">
            <?= 'Dijital Dosya'; ?>
            <small><?= "İzin Verilen Dosya Uzantıları"; ?>:&nbsp;<strong class="font-500"><?= $exts; ?></strong></small>
        </h4>
    </div>
    <?php if ($productSettings->digital_external_link == 1): ?>
        <ul class="nav nav-tabs nav-tabs-digital-file">
            <li <?= empty($product->digital_file_download_link) ? 'class="active"' : ''; ?>><a data-toggle="tab" href="#tabContentUploadFile"><?= "Dosya Yükle"; ?></a></li>
            <li <?= !empty($product->digital_file_download_link) ? 'class="active"' : ''; ?>><a data-toggle="tab" href="#tabContentAddUrl"><?= "Dış İndirme Bağlantısı Ekle"; ?></a></li>
        </ul>
        <div class="tab-content">
            ...
            <div id="tabContentAddUrl" class="tab-pane fade<?= !empty($product->digital_file_download_link) ? ' in active' : ''; ?>">
                <div class="form-box-body">
                    <input type="text" name="digital_file_download_link" class="form-control form-input" value="<?= esc($product->digital_file_download_link); ?>" placeholder="<?= "Dış İndirme Bağlantısı"; ?>" maxlength="499" onchange="$('#inputDigitalFileLink').val($(this).val());">
                </div>
                <p class="images-exp m-t-10"><i class="icon-exclamation-circle"></i><?= "Uyarı: Dış indirme bağlantısı"; ?></p>
            </div>
        </div>
    <?php else: ?>
        <div class="form-box-body">
    ...
    $(document).bind('ready ajaxComplete', function () {
        $('#drag-and-drop-zone-digital-files').dmUploader({
            ...
            onFileExtError: function (file) {
                $(".error-message-file-upload").html("<?= 'Geçersiz dosya tipi'; ?>");
                setTimeout(function () {
