...
<?php else: ?>
    <div class="product-slider-container">
        <?php if (countItems($productImages) > 0): ?>
            <div class="left">
                <div class="product-slider-content">
                        ...
                        <?php foreach ($productImages as $image): ?>
                            <div class="item">
                                <div class="item-inner">
                                    <img src="<?= IMG_BASE64_1x1; ?>" data-lazy="<?= getProductImageURL($image, 'image_small'); ?>" alt="<?= !empty($productDetails) ? esc($productDetails->title) : ''; ?>">
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    <?php if (countItems($productImages) > 7): ?>
                        <div id="product-thumbnails-slider-nav" class="product-thumbnails-slider-nav">
                            <button class="prev" aria-label="btn-prev-product-slider-thumb"><i class="icon-arrow-up"></i></button>
                            <button class="next" aria-label="btn-next-product-slider-thumb"><i class="icon-arrow-down"></i></button>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="right">
                ...
                <div id="product_slider" class="product-slider gallery">
                    <?php if (!empty($productImages)):
                        foreach ($productImages as $image): ?>
                            <div class="item">
                                <a href="<?= getProductImageURL($image, 'image_big'); ?>" class="glightbox">
            ...
            <div class="row-custom text-center">
                <?php if (!empty($video)): ?>
                    <button class="btn btn-lg btn-video-preview" data-toggle="modal" data-target="#productVideoModal" aria-label="btn-view-audio"><i class="icon-play"></i><?= "Video"; ?></button>
                <?php endif;
                if (!empty($audio)): ?>
                    <button class="btn btn-lg btn-video-preview" data-toggle="modal" data-target="#productAudioModal" aria-label="btn-view-video"><i class="icon-music"></i><?= "Ses"; ?></button>
                <?php endif; ?>
            </div>
        </div>
    </div>
